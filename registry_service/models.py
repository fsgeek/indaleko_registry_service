...
def _validate_timestamp(
    ...
    if isinstance(value, AwareDatetime) or isinstance(value, datetime):
        return value
    # Parse ISO timestamp and ensure it has timezone info
    ts = datetime.fromisoformat(value)
    if ts.tzinfo is None:
        raise ValueError(f"Timestamp {ts} is not timezone-aware.")
    return ts
...
class LookupResponse(BaseModel):
    """Response model for reverse lookup: UUID → semantic identifier."""
    uuid: UUID
    category: str
    name: str
